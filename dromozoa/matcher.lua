...
function class:make_error_string(message, start_srcloc)
  return message .. (start_srcloc or self.start_srcloc):make_at_string()
end
...
function class:match_long_string()
  ...
  if self:match "%[%[" then
    if not self:match("\n?(.-)%]%]") then
      error(self:make_error_string "unfinished long string")
    end
  ...
  elseif self:match "%[(=+)" then
    ...
    if not self:match "%[" then
      error(self:make_error_string "invalid long string delimiter")
    end
  ...
  while true do
    ...
    elseif self:match "\\x" then
      if not self:match "%x%x" then
        error(self:make_error_string("hexadecimal digit expected", start_srcloc))
      end
      table.insert(value, string.char(tonumber(self._0, 16)))
    elseif self:match "\\(%d%d?%d?)" then
      local code = tonumber(self._1, 10)
      if code > 0xFF then
        error(self:make_error_string("decimal escape too large", start_srcloc))
      end
      table.insert(value, string.char(code))
    elseif self:match "\\u" then
      if not self:match "{" then
        error(self:make_error_string("missing '{'", start_srcloc))
      end
      ...
      while self:match "%x" do
        if code > 0x7FFFFFF then
          error(self:make_error_string("UTF-8 value too large", start_srcloc))
        end
    ...
    elseif self:match "\\" then
      error(self:make_error_string("invalid escape sequence", start_srcloc))
    else
      error(self:make_error_string("unfinished string", start_srcloc))
    end
  end
